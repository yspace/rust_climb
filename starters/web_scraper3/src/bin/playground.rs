...
mod _image {
    ...
    pub fn main() {
        ...
        // 缩放
        // self::scale_up_example();
        self::scale_down_example();
    }
    ...
    fn scale_up_example() {
        ...
        let tiny = image::open("assets/test.png").unwrap();
        for &(name, filter) in [
        ...
        .iter()
        {
            ...
            let scaled = tiny.resize(1080, 1080, filter);
            println!("Scaled by {} in {}", name, Elapsed::from(&timer));
            let mut output = File::create(&format!("assets/up2-{}.png", name)).unwrap();
            scaled.write_to(&mut output, ImageFormat::Png).unwrap();

            let timer = Instant::now();
            let scaled = tiny.resize(720, 720, filter);
            println!("Scaled by {} in {}", name, Elapsed::from(&timer));
            let mut output = File::create(&format!("assets/up3-{}.png", name)).unwrap();
            scaled.write_to(&mut output, ImageFormat::Png).unwrap();
        }
    }

    fn scale_down_example() {
        ...
        .iter()
        {
            ...
            println!("Scaled by {} in {}", name, Elapsed::from(&timer));
            let mut output = File::create(&format!("assets/test-{}.png", name)).unwrap();
            scaled.write_to(&mut output, ImageFormat::Png).unwrap();
        }
        ...
        for size in &[20_u32, 40, 100, 200, 400] {
            ...
            let mut output = File::create(format!("assets/test-thumb{}.png", size)).unwrap();
            scaled.write_to(&mut output, ImageFormat::Png).unwrap();
        }
    }
}
